// 梦幻庄园一致性全遍历审计（真实引擎驱动，覆盖全部展示事件的 title/text/keyNote/选项/epilogue）
// 规则：玩家未购置庄园（flags.neverlandType 为 undefined 或 'none'）时，
...
function runTraversal(seed, force) {
  ...
  while (guard++ < 4000) {
    ...
    if (NEVERLAND_RE.test(joined) && !WHITELIST[id] && !ownsNeverland(st)) {
      localViolations.push({ eventId: id, neverlandType: st.flags.neverlandType == null ? 'undefined' : st.flags.neverlandType, sample: joined.slice(0, 80) });
    }
...
var violationMap = {};
function record(v) {
  var key = v.eventId + '|' + v.neverlandType;
  if (!violationMap[key]) violationMap[key] = { eventId: v.eventId, neverlandType: v.neverlandType, count: 0, sample: v.sample };
...
console.log('=== 梦幻庄园一致性全遍历审计 ===');
console.log('有效局数=' + totalRuns + '（含定向永不购置 300 局），未购置提及违规总数=' + totalViolations + '，涉及组合=' + keys.length);
keys.forEach(function (k) {
  var v = violationMap[k];
  console.log('  [' + v.neverlandType + '] ' + v.eventId + ' × ' + v.count + '  样例: ' + v.sample.replace(/\n/g, ' '));
});
if (keys.length) {
  console.error('FAIL 庄园一致性审计：存在未购置状态下的庄园内容提及，需加 cond 门控或修文案');
  process.exit(1);
}
console.log('PASS 庄园一致性全遍历审计：未购置庄园时无任何庄园相关内容（含定向永不购置 300 局）');
