...
- [ ] **GDD 正文残留「18 结局」旧引用全量刷新**：§8.2 可达性矩阵标题、§7 等仍有旧计数；本次仅更新了状态块(§〇)与 §5.4。建议对 GDD 做一次「18→30 / 60→70 / 75→81」全局替换 + 通读。
- [ ] **彩蛋计数统一**：GDD 记「10 个彩蛋」、README 记「15 枚」，需以 `events.js` 实际数量为准统一（并核对 `ACH_*` 彩蛋成就数）。
- [ ] **`test/smoke.cjs` 单元覆盖扩展至 30 结局**：当前 `ucases` 仅列 18 个，未覆盖 7 假设线 + 5 安全网结局（BURNT_OUT/OVERWORKED/HOMEBODY/LONELY_KING/QUIET_LIFE）。建议动态遍历 `MJ.config.endings` 抽样断言。
- [ ] **`ACH_ALL_ENDINGS` 文案已修「25→30」**（config.js）。其余成就描述若仍引用旧结局数，需复查。

---

## 四、玩法 / 内容扩展待办（中，整合自 GDD §17 + 架空历史 spec）

- [ ] **P1 BP4/BP5 法律应对分支**：两次指控事件的多路径应对（中性、不诽谤在世个人，§17.15 红线）。
