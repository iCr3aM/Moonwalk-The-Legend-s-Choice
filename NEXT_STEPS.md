...
- **彩蛋 / 趣事**：彩蛋 **36**（含 3 枚密蛋）；趣事 **61**（均已审计可达）。
...
- **GDD 正文「18 结局」旧引用全量刷新**：状态块/§5.4/§7.1/§8.2 标题/§17 骨架/埋点表等当前态计数全部改为 30 结局 / 114 变体 / 36 彩蛋 / 61 趣事 / 成就 88；§8.2 矩阵正文补全 12 个新结局行（7 假设线 + 5 安全网）。历史 changelog 行的「18」按原意保留。
- **彩蛋/趣事/变体计数统一**：以源码/审计为准 — 变体 **114**、彩蛋 **36**（含 3 枚密蛋）、趣事 **61**、结局 **30**、成就 **88 项**（重复 id 已清除）。GDD/README 均已刷新。
