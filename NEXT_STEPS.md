...
- [ ] **结局海报两页密度优化**（排版类方案已试两版并撤回——纯排版/居中不解决观感，改为「添加实质内容」填空）：
  - ✅ 已保留：关键抉择**时间线遍历**（history 全量 key 抉择，1958-2026 分段均匀采样、段内取 keyWeight 最高，最多 9 条单列呈现）。
  - 候选内容方案（待拍板，可组合）：
    - **A 人生年轮（Face1）**：独白下方加水平时间轴 1958→2009，标 5-7 个关键节点（年+短名），唱片纹样底——主题契合度最高
    - **B 六维星环（Face1）**：六维属性小雷达图，放独白下方居中——可视化填充力强
    - **C 同行者剪影（Face2）**：尾声后加已结识合作者一行（图标+英文名），未结识灰色 ???——呼应人物志收集
    - **D 本程之最（Face2）**：三个「最」（最高属性 / 最贵一笔资产变动 / 最早关键抉择），小卡片一行
    - **E 元路线四相（Face2）**：四条元路线 meta 值横条
    - 成本：C/D/E 小（各 ~30 行 canvas）；A/B 中（雷达/时间轴绘制 ~60-80 行）
  - 撤回记录：2026-09-09 两版排版方案（动态居中/自适应字号/收束行/足迹行/行距放宽）已回退，仅保留时间线遍历。
