...
- [ ] **结局海报两页密度优化**（用户反馈：两页海报留白过多、显得空）。候选方案（待拍板后实施）：
  - 正面：独白/扩写尾段后仍剩大量空白——可将「人生关键词 + 传奇评分」从背面提至正面独白下方作收束行；或独白区改垂直居中 + 行距/字号随内容自适应（保持 `--fs-*` 体系）。
  - 背面：成就 emoji 行（最多 26 枚）与尾声后空白——可将「关键抉择」从固定 2×2 扩为按 keyWeight 全量展示（滚动/裁剪策略需定），或背面底部增加「本程趣事/彩蛋精选 3 条」。
  - 约束：海报为 canvas 绘制（`createPosterFace1/2`），改动需复跑 `_audit_poster_quotes`（zh/en ≤2 行）与 `_audit_rel_met`（人物提及门控同样作用于海报文案）。
