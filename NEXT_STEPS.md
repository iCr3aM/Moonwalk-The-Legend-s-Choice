...
- [x] **结局海报两页密度优化（2026-09-09 已落地第一版，待用户看效果再迭代）**：
  - 正面：正文块按内容高度**动态垂直居中**（偏上 38%），消除短文本时中段大片空白；正文后新增收束行（分隔线 + 人生关键词 · 传奇评分）。
  - 背面：属性行距 44→52、成就 emoji 行距 42→50；**关键抉择 4→6 条**（2×3 网格，按实际行数计高）；尾声行高 24→26；尾声后新增「人生足迹」行（结识 X/6 · 关键抉择 N 次 · 命运分岔 M 段，i18n `ui.posterFootprint`）。
  - 约束遵守：未触碰语录区（`_audit_poster_quotes` ≤2 行）与人物文案门控（`_audit_rel_met`）；npm test 28 门禁全绿。
