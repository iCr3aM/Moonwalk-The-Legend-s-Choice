...
(function () {
  ...
  MJ.triviaSystem = {
    ...
    defs: {
      TRIVIA_CHARITY:        { icon: '🤝', name: '匿名代付陌生人账单', desc: '你曾悄悄为排队的陌生人结清账单，不留姓名——善意于你，本就是日常。', cond: function (s) { return s.flags.healWorld || (s.meta.phil || 0) >= 3; } },
      ...
      TRIVIA_ONOMATOPOEIA:  { icon: '🎶', name: '用拟声词讲编曲', desc: '你说不清和弦时，就“咚呲哒哒”地比划给乐手听，他们竟真听懂了。', cond: function (s) { return (s.meta.artPath || 0) >= 3; } },
      TRIVIA_FANMAIL:       { icon: '✉️', name: '手写回信给歌迷', desc: '面对成山的来信，你挑出几封亲手回了字句，落款总是“Love, Michael”。', cond: function (s) { return (s.relations.fans || 0) >= 35; } },
      ...
      TRIVIA_THISISIT:      { icon: '🎬', name: '为《This Is It》逐帧打磨走位', desc: '五十场演唱会的每个走位，你都和编舞师一帧帧对过，哪怕身体已亮起红灯。', cond: function (s) { return s.flags.thisItHeld || s.flags.thisItFull || s.flags.thisItReduced; } },
      ...
      TRIVIA_PEACE:         { icon: '🌍', name: '在战乱之地抱起陌生孩童', desc: '镜头之外，你蹲下身把当地的孩子抱起来，那张照片从没用来宣传。', cond: function (s) { return (s.meta.phil || 0) >= 4; } },
      ...
      TRIVIA_DIALTONE:    { icon: '☎️', name: '拨号音里的节拍', desc: '你对着拨号音“嘟——嘟——”打拍子，电话那头以为是线路故障，你却笑出了声。', cond: function (s) { return (s.meta.artPath || 0) >= 1 || (s.attributes.reputation || 0) >= 75; } },
      TRIVIA_GLOVE:       { icon: '🧤', name: '一只手套的魔法', desc: '那只闪着光的单只手套，是你给自己设的暗号：只要戴上它，舞台就只属于你一个人。', cond: function (s) { return s.flags.thriller25 === true || (s.meta.artPath || 0) >= 1 || (s.attributes.art || 0) >= 65; } },
      TRIVIA_QUIETSTAGE:  { icon: '🪑', name: '谢幕后的安静', desc: '掌声散尽，你独自坐在空荡的舞台边，听见自己的呼吸——那是最诚实的掌声。', cond: function (s) { return (s.attributes.stress || 0) <= 25; } },
      TRIVIA_MOTHERSONG:  { icon: '🎵', name: '唱给妈妈听', desc: '有次你随口哼起妈妈最爱的那首老歌，唱到一半，喉咙忽然发紧。', cond: function (s) { return (s.attributes.family || 0) >= 82; } },
      TRIVIA_HEALPLANET:  { icon: '🌍', name: '把地球缝补起来', desc: '你相信音乐能缝补裂痕：把不同肤色、不同语言的人，缝进同一段旋律里。', cond: function (s) { return s.flags.healWorld === true || (s.meta.phil || 0) >= 4; } },
      ...
      TRIVIA_ROBOT_DANCE:    { icon: '🤖', name: '机器人舞步', desc: '1974 年《Dancing Machine》在 Soul Train 引爆“机器人舞”，那套机械律动后来成了他的招牌。', cond: function (s) { return (s.meta.artPath || 0) >= 1 || (s.attributes.art || 0) >= 60; } },
      TRIVIA_MOONWALK_BOOK:  { icon: '📖', name: '自传《Moonwalk》', desc: '1988 年他出版个人自传《Moonwalk》，把聚光灯外的童年与心事写进了书页。', cond: function (s) { return (s.meta.artPath || 0) >= 1 || (s.attributes.art || 0) >= 65; } },
      TRIVIA_DANCING_DREAM:  { icon: '📝', name: '诗集《Dancing the Dream》', desc: '1992 年他出版诗集《Dancing the Dream》，字里行间是一个孩子未曾熄灭的想象。', cond: function (s) { return (s.meta.artPath || 0) >= 1 || (s.attributes.art || 0) >= 65; } },
      TRIVIA_THRILLER_SHORT: { icon: '🎬', name: '《Thriller》短片', desc: '1983 年的《Thriller》MV 是开创性的叙事恐怖短片，重塑了音乐录影带的形态。', cond: function (s) { return s.flags.thriller25 === true || (s.attributes.art || 0) >= 65; } },
      TRIVIA_JANET_SCREAM:   { icon: '👩', name: '与 Janet 的《Scream》', desc: '妹妹 Janet 同为巨星，1995 年二人合作《Scream》，荧幕上的兄妹对唱成了经典。', cond: function (s) { return (s.relations.brothers || 0) >= 10 || (s.attributes.reputation || 0) >= 75; } },
      ...
      TRIVIA_WEMBLEY:        { icon: '🏟️', name: '温布利之夜', desc: '1988.7.16 的温布利演唱会（Bad World Tour）成为传奇现场，数万人的合唱响彻夜空。', cond: function (s) { return (s.meta.artPath || 0) >= 1 || (s.attributes.art || 0) >= 70; } },
      TRIVIA_30TH_ANNIV:     { icon: '🎉', name: '30 周年庆典', desc: '2001 年的“30 周年庆典”在纽约麦迪逊广场花园举行，故人新朋同台致敬。', cond: function (s) { return (s.meta.artPath || 0) >= 1 || (s.attributes.art || 0) >= 70 || (s.attributes.reputation || 0) >= 75; } },
      TRIVIA_THRILLER25:     { icon: '💿', name: '《Thriller 25》', desc: '2008 年的《Thriller 25》是 25 周年纪念专辑，收录了未发表的曲目。', cond: function (s) { return s.flags.thriller25 === true || (s.attributes.art || 0) >= 65; } },
      ...
      TRIVIA_KING_OF_POP:    { icon: '👑', name: '流行之王', desc: '他被冠以“流行之王（King of Pop）”之名，是 20 世纪最具文化影响力的音乐人之一。', cond: function (s) { return (s.attributes.reputation || 0) >= 78; } },
      TRIVIA_BEN_SOLO1:      { icon: '🐶', name: '《Ben》登顶 Solo', desc: '1972 年的《Ben》是他首支个人冠军单曲，少年的嗓音第一次独自站上榜首。', cond: function (s) { return (s.meta.artPath || 0) >= 1 || (s.attributes.art || 0) >= 58; } },
      TRIVIA_ATV_CATALOG:    { icon: '📜', name: 'ATV 版权版图', desc: '1985 年他购入 Beatles 与 ATV 曲库版权，把旋律变成了可传承的资产。', cond: function (s) { return (s.attributes.wealth || 0) >= 70 || (s.meta.mogul || 0) >= 1 || (s.attributes.reputation || 0) >= 75; } },
      TRIVIA_BEATIT_GANGS:   { icon: '🤝', name: '《Beat It》的真实面孔', desc: '《Beat It》请来真实的帮派青年出演，用舞蹈代替对抗，唱出反暴力的姿态。', cond: function (s) { return (s.meta.artPath || 0) >= 1 || (s.attributes.art || 0) >= 60; } },
      TRIVIA_BLACKORWHITE_PREMIERE: { icon: '📺', name: '《Black or White》首播', desc: '1991 年《Black or White》全球首播，约一亿观众同时守在屏幕前。', cond: function (s) { return s.flags.blackOrWhite === true || (s.attributes.reputation || 0) >= 72 || (s.meta.artPath || 0) >= 1; } },
      ...
      TRIVIA_BILLIE_JEAN_MTV: { icon: '📺', name: '《Billie Jean》破壁 MTV', desc: '1983 年，《Billie Jean》的黑白影像成为 MTV 首批大量播出的黑人艺人作品之一，悄悄改写了电视台的肤色边界。', cond: function (s) { return s.flags.thriller25 === true || (s.attributes.art || 0) >= 65; } },
      TRIVIA_DONATED_500M:   { icon: '💛', name: '匿名捐出约五亿美元', desc: '据公开估算，你一生向慈善机构捐款约五亿美元，善意从不写在收据上。', cond: function (s) { return s.flags.healWorld === true || (s.meta.phil || 0) >= 3; } },
      TRIVIA_13_GRAMMY:     { icon: '🏆', name: '十三座格莱美', desc: '你职业生涯共获 13 座格莱美奖，其中包括传奇与终身成就类的加冕。', cond: function (s) { return (s.meta.grammyWins || 0) >= 1 || (s.attributes.reputation || 0) >= 82; } },
      TRIVIA_FIVE_DECADES:  { icon: '📈', name: '横跨五个十年的前十', desc: '你是史上首位在五个不同十年都进入 Billboard 前十的艺人，时间没能拦住旋律。', cond: function (s) { return (s.attributes.reputation || 0) >= 82 || (s.meta.artPath || 0) >= 3; } },
      TRIVIA_PEPSI_DEAL:    { icon: '🥤', name: '百事破纪录的代言', desc: '1984 年，你以约 500 万美元签下百事代言，刷新了当时艺人商业合作的纪录——镜头之外的故事，从不写进合同。', cond: function (s) { return s.flags.isSolo === true || (s.attributes.wealth || 0) >= 65; } }
    },
    _load: function () {
