...
- **其他**：成就 **56 项**（图鉴可重置）、关键选择回顾、社交分享（传奇海报 Canvas 保存/复制/分享）、**变体事件 65 个**（window+weight 年份窗口概率插入）、媒体/孤独属性、单文件构建与成就双发修复；**五图鉴统一**（结局/彩蛋/趣事/语录/成就 共用 `.gallery/.g-cell` 卡片组件）、**章节主题配色**（ch0–ch5 各设主色）、**响应式多断点**（600/640/900/1080）、**Toast 串行队列**。【§16.1 ✅】
- **本地化（EN 全量）**：65 变体 + 56 成就 + 8 彩蛋 + 24 趣事 + 12 语录 共 **318 字段**全部有 EN 词条（find_missing_en=0，i18n_coverage 测试通过）。【§13 P1 ✅】
...
- **五图鉴统一**：结局 / 彩蛋 / 趣事 / 语录 / 成就 共用同一 `.gallery/.g-cell` 卡片组件（锁定态 🔒+未解锁），维护面一致。
...
### 18.3 语录 / 彩蛋 / 趣事图鉴减列（长文自适应）
- **方案**：文案较长的语录/彩蛋/趣事图鉴，桌面 2 列、移动端 1 列，并放宽 `.g-desc` 高度，避免窄格挤行。
- **状态**：【已落地】`quote-gallery/egg-gallery/trivia-gallery` 减列 + 放长描述。
...
- **状态**：【规划】机制改动需评估对既有存档/测试影响；建议先做 ①+②（低风险、直接改善手感），再视效果调权。
...
- **状态**：【规划】建议本期实现最小可用版（记录+列表+缩略海报回看）。
