...
# 十七、后续扩展
> 实现状态（截至代码侧 v1.x）：成就（✓ 11 项）、关键选择回顾（✓）、社交分享（✓ 文案复制/系统分享/微博·X）、被搁置彩蛋 chinaVisit 访华（✓ 隐藏变体）、变体事件池至 20+（✓ 30+ 含隐藏/条件变体）、元路线倾向反馈（✓ §9）、生涯数据深度（✓ 变体/关键抉择计数）、环境音（✓ §10 WebAudio 开关）。**仅「多语言 i18n」一项尚待规划实现。**

- 多语言 i18n（待规划）
- 成就（慈善家/巡演王/法律斗士/隐士，✓ 已扩至 11 项）
- 关键选择回顾（✓）
- 社交分享（✓）
