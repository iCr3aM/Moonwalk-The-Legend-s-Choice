...
# 十七、后续扩展
> 实现状态（截至代码侧多轮迭代）：成就（✓ 30 项）、关键选择回顾（✓）、社交分享（✓ 传奇海报：Canvas 图片 + 保存/复制/分享图片 + 引导页文字分享）、被搁置彩蛋 chinaVisit 访华（✓ 隐藏变体）、变体事件池（✓ 34 个，含 5 个稀有门控隐藏/条件变体）、元路线倾向反馈（✓ §9）、生涯数据深度（✓ 节点/变体/关键抉择计数）、环境音（✓ §10 WebAudio 开关）。**功能性扩展大多已落地；余下「多语言 i18n」待规划，且仍有大量「体验深化（代入感与故事性）」候选，见 §17.1。**
