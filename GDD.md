...
> 实现：数据层事件加 `variant:true` + `weight`，`EventEngine` 在章节切换时按随机数决定是否插入。保证单周目主线完整，但“纹理”各异。
...
# 十七、后续扩展
> 实现状态（截至代码侧 v2.x）：成就（✓ 15 项）、关键选择回顾（✓）、社交分享（✓ 传奇海报：Canvas 图片 + 保存/复制/分享图片 + 引导页文字分享）、被搁置彩蛋 chinaVisit 访华（✓ 隐藏变体）、变体事件池至 20+（✓ 33+ 含隐藏/条件变体）、元路线倾向反馈（✓ §9）、生涯数据深度（✓ 节点/变体/关键抉择计数）、环境音（✓ §10 WebAudio 开关）。**仅「多语言 i18n」一项尚待规划实现。**
...
- 继续补充变体事件池至 20+（✓ 已达 33+，含 5 个稀有门控隐藏/条件变体）
...
*文档结束 — v0.5 深度拓展（选项/分支/变体系统/元路线/12 结局）*
