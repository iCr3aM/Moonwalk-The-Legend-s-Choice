...
### 已落地（代码侧已验证）
- **结局体系**：30 种（含 1 隐藏终极 `END_TRUE_ETERNAL` + 7 假设线 `END_ALT_*`），§7.2 优先级规则表唯一判定。
...
- **彩蛋系统**：10 个彩蛋 + 跨周目持久化 + 图鉴 + Toast。【§17.9 ✅】
...
- **其他**：成就（以 `config.js` 为准，约 80+ 项，图鉴可重置）、关键选择回顾、社交分享（传奇海报 Canvas 保存/复制/分享）、**变体事件 114 个**（window+weight 年份窗口概率插入）、媒体/孤独属性、单文件构建与成就双发修复；**四图鉴统一**（结局/彩蛋/趣事/成就 共用 `.gallery/.g-cell` 卡片组件；语录图鉴已移除）、**章节主题配色**（ch0–ch5 各设主色）、**响应式多断点**（600/640/900/1080）、**Toast 串行队列**。【§16.1 ✅】
- **本地化（EN 全量）**：114 变体 + 80+ 成就 + 36 彩蛋 + 61 趣事，EN 字段全量覆盖（find_missing_en=0，i18n_coverage 测试通过；语录图鉴已移除，原 12 语录不再计入）。【§13 P1 ✅】
...
## 5.4 结局系统
30 种结局（含 1 隐藏终极 + 7 假设线），标志优先 + 属性阈值 + 元路线（见第七章）。
...
## 5.7 重玩性设计
30 结局（含 1 隐藏终极 + 7 假设线）+ 变体事件 + 元路线 + 隐藏/条件事件 + 速通（1969 留盖瑞）+ 关键选择回顾。
...
| 七、告别与归宿 | 2006–2009 | 7_0,7_1,7_2,7_3 + 30 结局 |
| 八、续章·在场的不朽 | 2010–2026 | 8_0,8_1,8_2,8_3,8_4,8_5,8_6（仅 7_2 选「续写人生」进入） |

> 含变体事件与判定，总节点约 **92+**（主线 7 章 + 续章 7 节点 + 114 变体），达成设计目标 50–70+ 的饱和态。
...
| END_ALT_STAY_MOTOWN | tl['1975']==='motown'（BP1 留摩城） | 单飞节点选留 Motown，与兄弟共度 | rare（假设线） |
| END_ALT_NO_QJ | tl['1979']==='solo_prod'（BP2 独立制作） | 不与昆西合作、自掌创作 | rare（假设线） |
| END_ALT_HEALED | tl['1984']==='safe'（BP3 稳妥康复） | 百事烧伤后稳妥康复（非烧伤态） | rare（假设线） |
| END_ALT_MEDIA_MOGUL | tl['biz']==='empire'（BP6 传媒帝国） | 创立媒体帝国、扩张版图 | epic（假设线） |
| END_ALT_PEACE_LAUREATE | f.altPeace===true（和平路径） | 以慈善转身登顶慈善家之巅 | epic（假设线） |
| END_ALT_QUIET_RETIREE | f.altQuietRetiree===true（归隐路径） | 家庭稳固、主动退隐庄园 | rare（假设线） |
| END_ALT_SURVIVE_LEGACY | survived2009 & tl['2009']==='survive'（BP7 长寿） | 续章·假设未离世且走长寿分叉 | legendary（假设线） |
| END_HOMEBODY | isSolo & family≥70 & !debt & !burned | 单飞后仍把家庭经营到极致 | rare |
| END_LONELY_KING | loneliness≥70 & dom≠recluse & health≥40 & !debt & !burned | 非隐士路线却孤独极高 | rare |
| END_OVERWORKED | stress≥85 & health<50 & !debt & !burned & !dependent | 压力轴过载、身心透支 | rare |
| END_BURNT_OUT | reputation≥80 & health<42 & !debt & !burned & !dependent | 声誉极高却健康低（非烧伤/依赖/负债） | epic |
| END_QUIET_LIFE | 中性兜底（无专属归宿时收束） | 平稳谢幕的清白人生 | uncommon |

> 稀有度档位按「设计可达性 / 可达成感」进一步下调（用户反馈：稀有结局不应让玩家觉得遥不可及），并配合图鉴「如何达成」配方。`resolveEnding` 已于 2026-09-05 重构为**轨迹优先**：烧伤 / 负债 轨迹先判定（避免被"成功型结局"抢占而恒为 0），再处理 `!burned&!debt` 的成功/普通结局；各路线型结局要求该路线为**主导**（dom），使"无主导路线且健康声誉俱佳"的局流向 `PERFECT` 兜底。5000 局随机分布：STATESMAN≈2.3% / ART_PEAK≈1.3% / TRAGIC≈2.1% / INNOVATOR≈3.9% / MOGUL≈4.1% / 负债结局≈5%，**多数随机出现**（缺 PERFECT=刻意"非慈善清白人生"路径、部分假设线=续章/分叉稀有分支、TRUE_ETERNAL=隐藏终极≈0.04%，均属设计预期）；`check_endings_new.cjs` 定向用例保证 30/30 可达，`check_balance_reach.cjs` 仅作信息型报告、不硬 FAIL。
...
- **结局页**：名称 + 图标 + 基调 + 100~150 字独白 + 按基调注入 MJ 语录 + 属性快照 + **传奇海报（Canvas，可保存（长按）图片，底部署名 Cr3aM 制作 · MJ Forever）** + 四图鉴入口 + 人生回顾分组（专项造诣 / 关键抉择 / 人生手记 / 假如…(想象) / 时代回响 / 时代切片）。
- **Toast 串行队列**：成就 / 彩蛋 / 趣事 / 语录 解锁弹窗排队展示，避免重叠。
- **响应式**：多断点（≤600 / ≤640 / ≥900 / ≥1080），窄屏重排（padding/字号缩小、`.btn-row` 纵向、`.bars`/图鉴网格 2 列、菜单 2 列、`.toolbar` 换行、`.subdim-name` 限宽）；`prefers-reduced-motion` 尊重；模态 `max-height`+滚动。
...
### 18.1 GDD 对齐游戏现状（本次）
- §〇 实现状态总览更新：成就 60 / 变体 75 / EN 全量 356 字段 / 四图鉴统一 / 章节主题配色 / 响应式多断点；§8.2 由"十四结局矩阵"扩为"十八结局可达性矩阵"（补 STATESMAN/INNOVATOR/MENTOR/RECLUSE_SERENE）。【已落地（文档侧）】
...
# 十二、新手引导 / 十四、数据埋点
（规划同前；埋点新增：变体事件触发率、元路线分布、30 结局分布、三选占比。）
...
- **P2（事件 EN 全集）**：`events.js` 全部 `narr/text/label/hint/epilogue` 抽取并译 EN（约 90+ 主线节点 + 114 变体）。
...
> 目标：在已搭好的"数据驱动 + 元路线 + 114 变体 + 30 结局（含 1 隐藏终极 + 7 假设线）+ 海报"骨架上，补上**情感与叙事血肉**，强化"蝴蝶效应"（§2.2）与"历史尊重/情感真实"（§15）。按性价比分三档；每个模块若实现，建议在 GDD 中新增对应小节（如 §5.8/§5.9/§6.2 强化）。
...
> 在现有"6 属性 + 4 元路线 + 114 变体 + 30 结局"骨架上增加**可操作的策略层**，让选择从"三选其一"升级为"经营一条人生线"。
...
> 目标：让"集齐 30 结局 + 80+ 成就"之外，仍有持续回来玩的理由。
...
- **元彩蛋（第四面墙）**：集齐 80+ 成就后解锁"开发者留言"独白；连续 5 周目完成解锁"致每一位重写传奇的你"旁白。
...
- **新增成就候选（建议 id / 名称 / 稀有度 / 触发 sketch）**：
  ...
  | ACH_ALL_ENDINGS | 万相皆我 | legendary | 图鉴解锁全部 30 结局 |
